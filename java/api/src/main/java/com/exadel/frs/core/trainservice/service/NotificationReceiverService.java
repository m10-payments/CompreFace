...
public class NotificationReceiverService {
    ...
    void synchronizeCacheWithNotification(String payload) {
        try {
            ...
            if (SERVER_UUID.equals(cacheActionDto.serverUUID())) {
                return;
            }
            if (Objects.isNull(cacheActionDto.serverUUID())) {
                log.warn("Received notification with empty serverUUID: {}", cacheActionDto);
                return;
            }
            if (StringUtils.isBlank(cacheActionDto.apiKey())) {
                log.warn("Received notification with blank api key: {}", cacheActionDto);
                return;
            }
            if (Objects.isNull(cacheActionDto.cacheAction())) {
    ...
    private void processNotification(CacheActionDto<Map<String, Object>> action) {
        switch (action.cacheAction()) {
            case INVALIDATE -> notificationHandler.invalidate(action);
            case REMOVE_SUBJECTS -> notificationHandler.removeSubjects(convert(action));
            case ADD_EMBEDDINGS -> notificationHandler.addEmbeddings(convert(action));
            case REMOVE_EMBEDDINGS -> notificationHandler.removeEmbeddings(convert(action));
            case RENAME_SUBJECTS -> notificationHandler.renameSubjects(convert(action));
            case DELETE -> notificationHandler.handleDelete(action);
            case UPDATE -> notificationHandler.handleUpdate(action);
            default -> log.error("Can't process action with actionType={}", action.cacheAction());
        }
    }
    ...
    private <T> CacheActionDto<T> convert(CacheActionDto<Map<String, Object>> action) {
        return action.withPayload(
            objectMapper.convertValue(action.payload(), new TypeReference<>() {})
        );
    }
}
